...
def index(slug=None):
    ...
    with cursor() as dbc:
        ...
        if res:
            ...
            print(cfg)
            with cursor() as dbc:
                dbc.execute("""UPDATE main.urls set cfg=? where slug=?""", (json.dumps(cfg), slug))
...
def url():
    ...
    try:
        ...
        with cursor() as dbc:
            res = dbc.execute("INSERT INTO main.urls(cfg, created) values(?,?)", (json.dumps(targets), created))
            _id = res.lastrowid
